...
for k, v in {
    ...
    "project_name": "", "client_name": "", "project_number": "", "project_location": "", "revision": "Rev A",
...
def simulate_extraction(fnames):
    ...
    for i in range(12):
        rows.append({
            ...
            "RFI": "⚠ Check" if random.random() < 0.35 else "",
        })
...
def page_dashboard():
    ...
    c1.metric("Total Estimates", len(st.session_state.estimates))
...
def page_wizard():
    ...
    if step == 1:
        ...
        st.session_state.project_name = st.text_input("Project Name", st.session_state.project_name or "Coomera Project")
        st.session_state.client_name = st.text_input("Client", st.session_state.client_name or "Booga")
        if st.button("Next →"):
        ...
        st.number_input("Markup %", value=st.session_state.markup_pct, key="markup")
        st.number_input("Contingency %", value=st.session_state.contingency_pct, key="contingency")
